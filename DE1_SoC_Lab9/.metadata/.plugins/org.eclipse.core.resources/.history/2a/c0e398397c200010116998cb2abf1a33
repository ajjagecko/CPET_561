...
{
	uint32 switch_val = *(SwPtr);
	//Mask logic for setting flags
	if (1 == (switch_val & 0x01)) {
		//write 0 to addr 1 to turn on lpf
		*(FilterPtr + 1) = 0x0000;
		//set filter_flag to 1
		filter_flag = 1;
	}
	else if (1 <= switch_val) {
		//write 1 to addr 1 to turn on hpf
		*(FilterPtr + 1) = 0xFFFF;
		//set filter_flag to 1
		filter_flag = 1;
	}
	else {
		//set filter_flag to 0
		filter_flag = 0;
	}
	  ...
	  uint16 right_sample, left_sample;
        ...
        *PinPtr = TOGGLE;
	    ...
	    {   
	        ...
	        if (1 == filter_flag) {
				//write left_sample to data_i
				*FilterPtr = left_sample;
				//read filtered left_sample from data_o
	    	...
	    	if (CHANNELS == 2)                       //only read right sample if stereo mode
	    	{
				...
				if (1 == filter_flag) {
				   //write right_sample to data_i
				   *FilterPtr = right_sample;
				   //read filtered right_sample from data_o
